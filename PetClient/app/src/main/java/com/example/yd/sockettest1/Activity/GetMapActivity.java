...
public class GetMapActivity extends AppCompatActivity implements View.OnClickListener, NearbySearch.NearbyListener, AMapLocationListener, LocationSource, AMap.OnMarkerClickListener {
    ...
    private String myUserId = User.user_id;
...
//        mChangeUserButton.setOnClickListener(this);
    ...
    private void uploadlocation() {
        ...
        if (mlcoation != null){
            ...
            Toast.makeText(GetMapActivity.this, "定位位置为空", Toast.LENGTH_SHORT).show();
        }
        //设置上传用户id，一个NearbySearch单一只有唯一的ID，上传的位置需要保证userID是一样的
        loadInfo.setUserID(myUserId);
        //调用异步上传接口
        NearbySearch.getInstance(getApplicationContext())
    ...
    public boolean onMarkerClick(Marker marker) {
        ...
        intent = new Intent(this, DisplayFamilyActivity.class);
        intent.putExtra("toUserId", nearbyinfo.getUserID());
        intent.putExtra("myUserId",getMyId());
